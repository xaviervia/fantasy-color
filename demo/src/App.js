...
const SpacerHorizontal = ({small, medium}) => <div
  style={{
    width: elegir(
      medium, 15,
      small, 5
...
const SpacerVertical = ({small, medium}) => <div
  style={{
    height: elegir(
      medium, 15,
      small, 5
...
const Swatch = ({color}) => <svg width={80} height={80}>
...
const App = ({
  ...
  ? (
    <Container>
      ...
      <SpacerVertical medium />
      <BodyText>Current color <Input autoFocus onChange={onColorChange} value={colorWhileEditing} /></BodyText>
      <SpacerVertical small />
      <BodyText>Brightness <Strong>{Math.floor(brightness * 100) / 100}</Strong> / 255</BodyText>
      <BodyText>Equivalently bright shade <Strong>{equivalentBrightnessFantasyColor.toHEX()}</Strong></BodyText>
      <SpacerVertical medium />
...
export default withStore(
  ...
  (dispatch) => ({
    onColorChange: ({target: {value}}) => dispatch({
      type: 'COLOR_CHANGE',
      payload: value
    })
  })
